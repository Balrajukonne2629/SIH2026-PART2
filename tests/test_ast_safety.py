...
def test_clean_files_pass_ast_safety():
    ...
    assert ast_safety.assert_no_execution_imports(src_dir / "ast_safety.py") is True
    assert ast_safety.assert_no_execution_imports(src_dir / "remediation_engine.py") is True
    assert ast_safety.assert_no_execution_imports(src_dir / "main.py") is True
    assert ast_safety.assert_no_execution_imports(src_dir / "vendor_adapter.py") is True
    assert ast_safety.assert_no_execution_imports(src_dir / "vendor_registry.py") is True
    assert ast_safety.assert_no_execution_imports(src_dir / "juniper_auditor.py") is True
